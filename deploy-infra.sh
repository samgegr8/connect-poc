...
while getopts "n:f:t:e:m:p:o:c:d:s:q:y:x:a:h" opt; do
  case $opt in
    ...
    f)
      ...
      if [[ ! -z ${TEMPLATE_OVERRIDE_PATH} ]]; then
...
GIT_REPOSITORY=$(git config --get remote.origin.url 2>/dev/null) #Will be empty if a different remote name is set, but that's unlikely.
ACCOUNT=$(aws iam list-account-aliases --query 'AccountAliases[0]' --output text)
ACCOUNTNR=$(aws sts get-caller-identity --query 'Account' --output text)
...
function checkDeployment {
    ...
    if [[ ${RESULT} == 0 ]]; then
      ...
      if [[ ("${CHANGESET_TYPE}" == "CREATE") && (-z $DEPLOY_CHANGESET) ]]; then
        ...
        while true; do
          read -p "Because this is a new stack, you have to delete it before you can rebuild it. Do you wish to do so now? " yn
          case $yn in
...
if [[ ! -z $COMMON_PARAMETER_FILE ]]; then
...
aws cloudformation create-change-set ${TEMPLATE_FILE} --change-set-type ${CHANGESET_TYPE} --stack-name "${NAME}" --change-set-name "${CHANGESETNAME}" ${PARAMETER_FILE_STRING}  --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM CAPABILITY_AUTO_EXPAND >/dev/null
if [[ "$?" -ne 0 ]]; then
...
# Add the changeset
escapedchanges=$(aws cloudformation describe-change-set --change-set-name ${CHANGESETNAME} --stack-name ${NAME} --query 'Changes' --output json | jq '. | tostring')
...
while true; do
    read -p "Do you wish to apply this changeset? " yn
    case $yn in
        [Yy]* ) break;;
